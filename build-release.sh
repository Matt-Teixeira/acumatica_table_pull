#!/usr/bin/env bash
# Release acumatica_sync: mirror THIS working tree to /opt/apps/<APP_NAME>,
...
#   5. chown + build as svc  — image becomes acu-sync:svc
set -euo pipefail
...
# cache. This app has NO gitignored bulk (verified: git status --ignored shows
# only .env) and no file writers, so the exclude list is short: things that
# must never ship (git, agent dirs) plus the historical .gitignore patterns
# (*.log/*.pem/*.crt, deltaStore, migrationQueries, db/backup) as cheap
# insurance against local junk. No tracked file matches any of these
# (verified via git ls-files). Verify exclude changes by diffing `tar -tf`
# output, never by eyeballing patterns.
sudo mkdir -p "$DEST"
...
# index.js prints this on every boot line, so every run's console output
# records the commit that produced it. No release_sha column on the shared
# stats.job_runs table (standing fleet decision) — provenance lives here and
# on the boot line. Never set by hand; a dev tree has no key and prints
# 'dev-tree' instead.
...
echo "==> release complete: $DEST  commit: $GIT_SHA  image: acu-sync:svc"
